import { IDomainNames } from "~/types/response.type";
...
export function createDomainListKey(domainName: IDomainNames, filters?: Record<string, any>): string {
	const filterObject = keyGenerator(filters);
...
export function createDomainSingleKey(domainName: IDomainNames, id: string | number): string {
	return JSON.stringify({
		domain: domainName,
		id: id,
	});
